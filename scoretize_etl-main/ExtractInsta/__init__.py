...
from ExtractInsta.instagram import instagram
import pandas as pd
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        final_df = instagram(input_config, api_config)

        azs.upload_blob_df(final_df, path)

        return func.HttpResponse(f"{function_name} csv created: {path}")
    except Exception as e:
        logging.error(f"ExtractInsta Init File Error: {e}")
        return func.HttpResponse(f"ExtractInsta Init File Error: {e}")
