...
def get_likes_followers(urls,domains):
    res=[]
    try:
        logging.info(f"Extracable Url: {urls}")
        for j in urls:
            for i in domains:
                ...
                try:
                    ...
                    for item in res1:
                        for k, v in item.items():
                            fb_url = k
                            likes = v['follower_count'] 
                            followers = v["page_likers"]["global_likers_count"]
                            print(likes,followers)
                            output = {'likes':likes,'followers':followers}
                            res.append(output)
                            logging.info(f"Fb Url Response: {res}")
                    ...
                    response = requests.request("POST", url, json=payload, headers=headers)
                    res1 = response.json()["Results"]
                    # logging.info(f"Company Url Response: {res1}")
                    # fb_url = res['Facebook Page Data']
                    likes = res1['Facebook Page Data']['Follower Count']
                    followers = res1['Facebook Page Data']['Likes']
                    output = {'likes':likes,'followers':followers}
                    res.append(output)
                    logging.info(f"CompanyUrl Response: {res}")
                
            return res
    except Exception as e:
        output = {'url':fb_url,'likes':"erros",'followers':"errors"}
        res.append(output)
        logging.error(f"ExtractFB: {e}")
        return res
...
def response_to_df(res,urls):
    df_list=[]
    logging.info(f"Urls List: {urls}")
    i=0
    for j in res:
        
            try:
                temp = {"facebook": urls[i], "fb_likes": j['likes'], "fb_followers": j['followers']}
                df_list.append(temp)
                i+=1
            except Exception as e:
                temp = {"facebook": urls[i], "fb_likes": "errors", "fb_followers": "errors"}
                
                df_list.append(temp)
                logging.error(f"Response to Df: {e}")
...
def facebook(data_dict, config_dict, n_posts=5):
    ...
    try:
        ...
        domains = data_dict['site_list']

        res = get_likes_followers(facebook_df["facebook"].to_list(),domains)

        res_df = response_to_df(res,facebook_df["facebook"].to_list())
