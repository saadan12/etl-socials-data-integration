...
from ExtractYoutube.youtube import youtube
import pandas as pd
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        final_df = youtube(input_config, api_config)

        azs.upload_blob_df(final_df, path)

        return func.HttpResponse(f"{function_name} csv created: {path}")
    except Exception as e:
        logging.error(f"ExtractYoutube Init func. error: {e}")
        return func.HttpResponse(f"ExtractSpyfu Init func. error: {e}")
