...
def concatenate(datajson):
    try:
        pagespeed_path = "{0}/{1}/{1}_{2}_{0}.csv".format(
            datajson["timestamp"], "social_webscrapper", datajson["container_name"]
        )
        azs = AzureStorage(datajson["container_name"])
        folder_path = "{0}/{1}/".format(datajson["timestamp"], "social_webscrapper")
        ...
        while True:
            ...
            if n_files_path == n_links:
                ...
                logging.info(f"ExtractPageSpeed Concatenate df: {df} ")
        ...
        logging.error(
            f"SocialwEBScrapper Concatenate func. error: {e}, \n Input config: {datajson}"
        )
