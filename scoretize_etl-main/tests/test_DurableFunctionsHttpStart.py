...
class DurableFunctionsHttpStartTestCase(IsolatedAsyncioTestCase):
    """
    Fake class to test the DurableFunctionsHttpStart.main function
    """
    async def test_durablefunctionsorchestrator_trigger(self):
        ...
        mock_response = func.HttpResponse(
            body=None, status_code=200, headers={"Retry-After": 10}
        )
        ...
        with patch(
            ...
            self.assertEqual(str(10), response.headers["Retry-After"])
